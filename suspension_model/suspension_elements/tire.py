...
class Tire:
    ...
    def direction(self):
        ang_x, ang_y = self.kingpin.normalized_transform()
        x_rot = rotation_matrix(unit_vec=[1, 0, 0], theta=-1 * (ang_x - self.initial_kpi))
        y_rot = rotation_matrix(unit_vec=[0, 1, 0], theta=(ang_y - self.initial_caster))
        z_rot = rotation_matrix(unit_vec=[0, 0, 1], theta=self.induced_steer)
        direction = np.matmul(y_rot, np.matmul(x_rot, np.matmul(z_rot, self.initial_direction)))
    ...
    def center(self):
        ...
        center_position = np.matmul(y_rot, np.matmul(x_rot, self.center_to_kingpin)) + self.kingpin.inboard_node.position
