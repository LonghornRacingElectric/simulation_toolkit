...
file_name = 'simmons_junk_mess_brother_type_shit.csv'
with open(file_name) as f:
    data = pd.read_csv(f)

rpm_d = data['Wheel RPM']
torque_d = data['Scaled Combined Axle Torque [Nm]']
...
plt.scatter(rpm_d*rpm2vel, torque_d, label='axle torque potential states', c='k', s=3)
...
plt.plot(boundary[:, 0]*rpm2vel, boundary[:, 1], color='k', label='convex hull')
...
max_vel = 35  # [m/s]
rpm2vel = 2*np.pi*8*0.0254/60
coefficients = np.polyfit(rpm_d*rpm2vel, torque_d, deg=2)
grip_fit = np.poly1d(coefficients)
velocities = np.linspace(0, max_vel, mesh)
tire_grip = grip_fit(velocities)
pwr_lim = [80000/(velocities[i]/(8*0.0254)) for i in range(len(velocities))]
target = np.minimum(tire_grip, pwr_lim)

plt.figure(figsize=(10, 5))
plt.plot(velocities, target, color=(1, 0, 0), label='target wheel torque-speed curve', linewidth=5)
plt.plot(velocities, tire_grip, label='axle torque potential', color='k', linewidth=2, linestyle='--')
plt.plot(velocities, pwr_lim, color='g', label='power limit', linewidth=2, linestyle='--',
         alpha=0.7)
plt.ylim(min(tire_grip*0.8), max(tire_grip)*1.3)
plt.xlabel('velocity [m/s]')
plt.ylabel('Torque [Nm]')
plt.title('Axle Torque-Speed Target')
plt.legend()
plt.show()

output = np.column_stack((velocities, target))
