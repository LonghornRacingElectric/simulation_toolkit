from simulations.transient._transient_helpers.states_6dof import States6DOF
from simulations.transient._transient_helpers.states_3dof import States3DOF
...
class Transient:
    ...
    def __init__(self, model_path: str, use_mode: int = 1):
        if use_mode == 1:
            ...
            self.state = States6DOF(model_path=model_path)
        elif 3 <= use_mode <= 5:
...
if __name__ == "__main__":
    sim = Transient(model_path="./_1_model_inputs/Nightwatch.yml", use_mode=2)

    result = sim.ramp_steer(
        velX=50 * 0.2777778, # kph to mps
        ramp_height=15 * np.pi / 180,
        ramp_duration=3.5,
        ramp_offset=3.5,
        sim_duration=3.5 * 3,
        time_step=0.01,
