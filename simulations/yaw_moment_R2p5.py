...
class YMD:
    ...
    def _residual_function(self, x: Sequence[float], args: Sequence[float]) -> Sequence[float]:
        ...
        cg_pos = np.array([suspension.cgx_lookup(x=delta, y=heave, z=pitch, w=roll)[0],
                           ...
                           suspension.cgz_lookup(x=delta, y=heave, z=pitch, w=roll)[0]])

        self.FL_Cp_wrt_cg = FL_Cp_pos - cg_pos
        self.FR_Cp_wrt_cg = FR_Cp_pos - cg_pos
        self.RL_Cp_wrt_cg = RL_Cp_pos - cg_pos
        self.RR_Cp_wrt_cg = RR_Cp_pos - cg_pos
        
        FL_jounce = suspension.FL_total_jounce_lookup(x=delta, y=heave, z=pitch, w=roll)[0]
        FR_jounce = suspension.FR_total_jounce_lookup(x=delta, y=heave, z=pitch, w=roll)[0]
        RL_jounce = suspension.RL_total_jounce_lookup(x=delta, y=heave, z=pitch, w=roll)[0]
        RR_jounce = suspension.RR_total_jounce_lookup(x=delta, y=heave, z=pitch, w=roll)[0]
        ...
        Fr_lat_LT = (FL_double_wishbone.weight + FR_double_wishbone.weight) * ay / 9.81 * cg_pos[2] / abs(FL_Cp_pos[1] - FR_Cp_pos[1])
        Rr_lat_LT = (RL_double_wishbone.weight + RR_double_wishbone.weight) * ay / 9.81 * cg_pos[2] / abs(RL_Cp_pos[1] - RR_Cp_pos[1])
        left_long_LT = (FL_double_wishbone.weight + RL_double_wishbone.weight) * ax / 9.81 * cg_pos[2] / abs(FL_Cp_pos[0] - RL_Cp_pos[0])
        right_long_LT = (FR_double_wishbone.weight + RR_double_wishbone.weight) * ax / 9.81 * cg_pos[2] / abs(FR_Cp_pos[0] - RR_Cp_pos[0])
        ...
        # Fz_lst = [x if x > 0 else 0 for x in Fz_lst]
