...
class YMDConstantVelocity:
    ...
    def initialize_funcs(self) -> None:
        ...
        FL_spring_MRs = [self.kin_FMU["FL_spring_MR"](np.array([0, jounce, 0, 0])) for jounce in jounce_sweep]
        FR_spring_MRs = [self.kin_FMU["FR_spring_MR"](np.array([0, jounce, 0, 0])) for jounce in jounce_sweep]
        RL_spring_MRs = [self.kin_FMU["RL_spring_MR"](np.array([0, jounce, 0, 0])) for jounce in jounce_sweep]
        RR_spring_MRs = [self.kin_FMU["RR_spring_MR"](np.array([0, jounce, 0, 0])) for jounce in jounce_sweep]

        Fr_stabar_MRs = [self.kin_FMU["Fr_stabar_MR_rot"](np.array([0, 0, 0, roll])) for roll in roll_sweep]
        Rr_stabar_MRs = [self.kin_FMU["Rr_stabar_MR_rot"](np.array([0, 0, 0, roll])) for roll in roll_sweep]
