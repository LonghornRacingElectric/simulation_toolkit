...
StaticVector<double, 3UL> unit_vec(const StaticVector<double, 3UL> &p1, const StaticVector<double, 3UL> &p2) {
    StaticVector<double, 3UL> vector_AB = p2 - p1;
    double vector_AB_mag = norm(vector_AB);
    if (vector_AB_mag < 1e-10) {
        // Return zero vector if points are too close
        return StaticVector<double, 3UL>{0, 0, 0};
    }
